...
export async function resetPassword(
  ...
  try {
    ...
    const loginResult = await secureLoginUser(cleanUsername, currentPassword);
    
    if (!loginResult.success) {
      logSecurityEvent('PASSWORD_RESET_FAILED_AUTH', { username: cleanUsername });
    ...
    const { error } = await supabase
      .from('staff')
      .update({ password: newPassword }) // This will be hashed by the trigger
      .eq('username', cleanUsername);

    if (error) {
      logSecurityEvent('PASSWORD_RESET_DB_ERROR', { 
        username: cleanUsername, 
        error: error.message 
      });
      return { success: false, error: 'Lỗi cập nhật mật khẩu. Vui lòng thử lại.' };
    }
