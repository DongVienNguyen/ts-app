...
import { PieChart, Pie, Cell } from 'recharts'; // Explicitly import PieChart and Pie
import { DrillDownDialog } from './DrillDownDialog';
import { useStatisticsData } from '@/hooks/useStatisticsData';
import { StatisticsHeader } from './statistics/StatisticsHeader'; // Changed to relative path
import { KeyMetricsGrid } from './statistics/KeyMetricsGrid'; // Changed to relative path
import { ChartContainer } from './statistics/ChartContainer'; // Changed to relative path
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        <div className="lg:col-span-5">
          <ChartContainer title="Xu hướng giao dịch" icon={<LineChartIcon className="mr-2 h-5 w-5 text-indigo-600" />} onExport={() => handleExportChartData(chartData.transactionTrends, [{key: 'date', label: 'Ngày', type: 'text'}, {key: 'count', label: 'Số giao dịch', type: 'number'}], 'xu_huong_giao_dich')} isLoading={isLoading} height={350}>
        ...
        <div className="lg:col-span-3">
          <ChartContainer title="Giao dịch theo nhân viên" icon={<BarChartIcon className="mr-2 h-5 w-5 text-green-600" />} onExport={() => handleExportChartData(chartData.staffTransactionStats, [{key: 'name', label: 'Nhân viên', type: 'text'}, {key: 'count', label: 'Số giao dịch', type: 'number'}], 'giao_dich_nhan_vien')} isLoading={isLoading}>
        ...
        <div className="lg:col-span-2">
          <ChartContainer title="Phân bổ loại giao dịch" icon={<PieChartIcon className="mr-2 h-5 w-5 text-amber-600" />} onExport={() => handleExportChartData(chartData.transactionTypeStats, [{key: 'name', label: 'Loại giao dịch', type: 'text'}, {key: 'value', label: 'Số lượng', type: 'number'}], 'phan_bo_loai_giao_dich')} isLoading={isLoading}>
            <PieChart>
        ...
        <div className="lg:col-span-3">
          <ChartContainer title="Giao dịch theo phòng" icon={<BarChartIcon className="mr-2 h-5 w-5 text-cyan-600" />} onExport={() => handleExportChartData(chartData.roomTransactionStats, [{key: 'name', label: 'Phòng', type: 'text'}, {key: 'count', label: 'Số giao dịch', type: 'number'}], 'giao_dich_theo_phong')} isLoading={isLoading}>
        ...
        <div className="lg:col-span-2">
          <ChartContainer title="Giao dịch theo Ca" icon={<BarChartIcon className="mr-2 h-5 w-5 text-purple-600" />} onExport={() => handleExportChartData(chartData.partsDayTransactionStats, [{key: 'name', label: 'Ca', type: 'text'}, {key: 'count', label: 'Số giao dịch', type: 'number'}], 'giao_dich_theo_ca')} isLoading={isLoading}>
        ...
        <div className="lg:col-span-5">
          <ChartContainer title="Số lượng bản ghi" icon={<DatabaseIcon className="mr-2 h-5 w-5 text-blue-600" />} onExport={() => handleExportChartData(chartData.statistics, [{key: 'name', label: 'Bảng', type: 'text'}, {key: 'count', label: 'Số bản ghi', type: 'number'}], 'so_luong_ban_ghi')} isLoading={isLoading}>
